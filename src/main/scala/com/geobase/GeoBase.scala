...
class GeoBase() extends Serializable {
  ...
  /** Returns the cities associated to the given airport.
    ...
    * one city).
    *
    * The method getCityFor returns the first city corresponding to the given
  ...
  /** Returns the trip duration between two locations (airport or city).
    ...
    *
    * {{{
    * assert(geoBase.getTripDurationFromLocalDates(
  ...
  /** Returns the geo type of a trip (domestic, continental or inter continental).
    ...
    * @return the type of the trip (a GeoType enum value, such as GeoType.DOMESTIC).
    */
  def getGeoType(locations: List[String]): Try[GeoType] = {
    ...
    for {
      ...
      countries <- locations.traverse(loc => getCountryFor(loc))
      ...
      geoType <- distCountries.length match {
        ...
        case _ =>
          for {
            ...
            iataZones <- distCountries.traverse(ctry => getIataZoneFor(ctry))
